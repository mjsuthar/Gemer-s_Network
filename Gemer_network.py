...
def split_comma_list(string_input):
    # Takes input of comma separated values and splits them into a list
    items = []
    comma_pos = 0
    s = string_input
    while ',' in s:
        comma_pos = s.find(',')
        items.append(s[:comma_pos])
        s = s[comma_pos+2:]
    if len(s) > 0:
        items.append(s)
    return items
...
def string_filter(string, search):
    # Takes string and splits off the name and substring that goes to split_comma_list
    name = ''
    vals = []
    if search in string:
        pos = string.find(search)
        name = string[0:(pos - 1)]
        vals = split_comma_list(string[pos + len(search) + 1:])
    return name, vals


def create_data_structure(string_input):
    # network is a dictionary of format Name:[[Connections], [Games]]
    network = {}
    s = string_input
    # Split string by sentences to be run through string_filter and added to network
    while '.' in s:
        name = ''
        connections = []
        games = []
        ...
        temp = s.split()
        if temp[1] == 'is':
            name, connections = string_filter(s[:period_pos], 'is connected to')
            if name not in network:
                network[name] = [[],[]]
            network[name][0] = connections
        if temp[1] == 'likes':
            name, games = string_filter(s[:period_pos], 'likes to play')
            if name not in network:
                network[name] = [[],[]]
...
#   Finds the number of games that user_A and user_B have in common.
#
# Arguments:
#   network: the gamer network data structure
#   user_A:  a string containing the name of user_A
...
#
# Return:
#   The number of games in common (as an integer).
#   - If user_A or user_B is not in network, return False.
def games_in_common(network, user_A, user_B):
    if user_A in network and user_B in network:
        total = 0
        for game in network[user_A][1]:
            if game in network[user_B][1]:
                total += 1
        return total
    return False
...
#   Adds a game for user. Make sure to check that user
#   exists in network.
#
# Arguments:
#   network: the gamer network data structure
#   user:  a string with the name of the user
#   game:  a string with the game the user is adding
#
# Return:
#   The updated network with the new game added.
#   - If the game already exists for user, return network unchanged.
#   - If user is not in network, return False.
def add_connection(network, user, game):
    if user in network:
